...
func NewKafka(cfg *config.KafkaConfig, cg *infra.CloserGroup) (*Kafka, error) {
	version, err := sarama.ParseKafkaVersion(cfg.Version)
	if err != nil {
		log.Printf("warn: invalid kafka version %q, fallback to v3.5.0.0: %v", cfg.Version, err)
		version = sarama.V3_5_0_0
	}

	producer, err := createSyncProducer(cfg.Brokers, version)
	if err != nil {
